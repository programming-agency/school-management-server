...
const appliedStudentRoutes = (app) => {
    ...
    app.get('/api/appliedStudent/:id', async (req, res) => {
        ...
        try {
            ...
            if (findAppliedStudent) {
                res.status(200).json({ message: "Found student successfully", student: findAppliedStudent });
            } else {
    ...
    app.put("/api/appliedStudent/:id", async (req, res) => {
        ...
        try {
            const updateData = await appliedStudent.findByIdAndUpdate({ _id: id }, updateAppliedStudentData, { new: true });

            if (updateData) {
                return res.status(200).json({ message: "New student Data Update", student: updateData })
            } else {
